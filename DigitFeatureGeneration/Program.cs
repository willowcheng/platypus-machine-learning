		...
		static void Main()
		{
			...
			for (var i = 0; i < trainReference.Length; i++) 
			{
				if (i % 2 == 0 && i < trainReference.Length - 1) 
				{
					...
					algorithmExampleTrainResultString += Algorithm.algorithmExample (trainReference [i], Constants.trainingSetPath, trainReference [i + 1]) + "\n";
				}
			}
			...
			for (var i = 0; i < testReference.Length; i++) 
			{
				...
				{
					// Generating 56 features using summation for columns and rows
					algorithmExampleTestResultString += Algorithm.algorithmExample (testReference [i], Constants.testSetPath) + "\n";
				}
			}
